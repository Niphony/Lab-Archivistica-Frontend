...
\begin{itemize}
  ...
  \item \textbf{IMPORTANTE: } La ejecución de un script de bash en el manual se indica como \texttt{bash nombre\_comando}; la palabra \texttt{bash} equivale a \texttt{./nombre\_comando}. Tome siempre esto en cuenta a la hora de ejecutar un script. 
\end{itemize}
...
\begin{itemize}
  \item \texttt{dspace.server.url}: Locación del servicio de servidor de Dspace. 
    \item \texttt{dspace.ui.url}: Locación del servicio de frontend de Dspace. 
    \item \texttt{cors.allowed.origins / rest.cors.allowed-origins}: Ubicaciones que permite dspace para ser accedido desde ellas, necesitan cambiar junto con el certificado del sitio si se requiere.
\end{itemize}
...
\begin{itemize}
   \item \textbf{Nginx no me redirigue a donde necesito.} Detenga docker, y cambie el mapeo en \texttt{docker-compose.yml}, por la dirección necesitada incluida las variables necesarias.
  ...
  \item \textbf{DSpace tarda en iniciar.} La API ejecuta una migración de base
   ...
   \item \textbf{DSpace no carga al cambiar la dirección o certificado del sitio.} Revise dentro de la carpeta de dspace, el archivo local.cfg y cambie lo necesario, despúes vuelva a levantar el servicio.
